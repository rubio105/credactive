...
- **Doctor Alert Inline Response (November 10, 2025)**: Added "Rispondi" button directly in alert cards for immediate doctor response. Clicking opens dialog with alert context and textarea for quick reply. Response creates doctor note linked to alert via `alertId`, sends patient notification, and invalidates alert cache. UX optimized for single-click workflow: view alert → respond → send (no page navigation). Dialog shows alert description above response field for context retention.
- **Doctor Dashboard Grid Layout (November 10, 2025)**: Redesigned doctor dashboard to match patient dashboard UX with 6-card grid layout (2 per row, 3 rows). Cards: "Parla con CIRY", "Alert Pazienti" (critical conversations with badge), "I miei pazienti", "Prenotazioni", "Note e Referti" (document sending), "Condividi codice". Removed "Disponibilità" (accessible via BottomNavigation), "Impostazioni" and "Sicurezza" (accessible via user menu dropdown). Navigation simplified to single "AI Prevenzione" item, eliminating duplicate menu entries. All critical doctor workflows accessible via intuitive icon grid.
- **Doctor Dashboard Icon Clarity (November 10, 2025)**: Resolved duplicate "Agenda" confusion by renaming availability management to "Disponibilità" with Clock icon (previously ClipboardList). Changed "Prenotazioni" icon from Calendar to CalendarCheck for better semantic distinction between confirmed appointments vs. time slot management. BottomNavigation aligned: "Prenotazioni" (CalendarCheck) instead of "Agenda". Desktop/mobile navigation now semantically consistent.
- **Login Guard Enhancement (November 10, 2025)**: Added useEffect-based redirect guard in Login page. Users already authenticated are automatically redirected to role-appropriate dashboard (Admin → /admin, Doctor → /doctor/patients, Patient → /dashboard) preventing login form display post-authentication. Console logging added for debugging.
- **Logout Fix & Session Cleanup (November 10, 2025)**: Fixed critical logout bug where session persisted after logout due to client-side navigation. All logout handlers (navigation.tsx, AdminLayout.tsx, useLogout.ts, logout.tsx, corporate-portal.tsx, patient-ai.tsx, settings.tsx) now use `window.location.href = '/login'` for aggressive full page reload instead of wouter's setLocation(). Added comprehensive logging (`[LOGOUT]` prefix) for debugging. Ensures complete session invalidation: backend req.logout() + req.session.destroy() + res.clearCookie() + frontend queryClient.clear() + storage clear + full page reload.
