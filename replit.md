...
-   **Medical Prevention System (Prohmed Partnership)**: Comprehensive health prevention module powered by Google Gemini AI, featuring medical document upload/analysis (PDF parsing with AI topic extraction), prevention topic taxonomy with sensitive content flags, AI-powered educational prevention assistant ("AI Prohmed - Impara la Prevenzione") publicly accessible without registration, medical alert system for admin review, and Prohmed telemedicine app access code generation for premium users and challenge winners.
    ...
    -   **Prevention Invite System**: Admin-triggered email invitation system for promoting AI Prohmed prevention education. Admin endpoint `POST /api/admin/send-prevention-invite` allows sending personalized invitations with direct link to `/prevention` page. Email template (`sendPreventionInviteEmail`) features emerald/teal gradient design, explains AI Prohmed educational system, lists 4 key features (personalized conversation, evidence-based strategies, resources, privacy), includes both HTML and professional text versions for compatibility. Accessible from authenticated user home page via prominent hero card with "Inizia Ora" CTA button.
...
-   **Prohmed Code Management System**: Admin bulk code generation system for Prohmed telemedicine access. Features pool-based architecture where codes are generated without user assignment (nullable userId), admin panel for bulk generation (up to 100 codes per batch), code type selection (individual/family), status tracking (active/redeemed/expired), and distribution management. Endpoints: GET /api/prohmed-codes (admin list all), POST /api/prohmed-codes/generate (admin bulk generate), POST /api/prohmed/codes/generate (generate for specific user).
